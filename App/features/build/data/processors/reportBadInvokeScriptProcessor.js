    ...
    ctor.prototype.parseNode = function(idMap, sceneName, script, node) {
        if (node.currentScope && node.currentScope == 'Current') {
            ...
            if ("Main" == entryPointName) {
                this.report.log(sceneName + ' : ' +  script.name + ' (file: Composer/Data/Scripts/' + script.id + '.json)', "Current: Main detected.");
            } else if ('' == node.entryPointId || null == node.entryPointId || !node.entryPointId) {
                this.report.log(sceneName + ' : ' + script.name + ' (file: Composer/Data/Scripts/' + script.id + '.json)', "Current scope set, but no EntryPoint defined.");
            } else if (node.entryPointId && '' != node.entryPointId && null != node.entryPointId && !entryPointName) {
                this.report.log(sceneName + ' : ' +  script.name + ' (file: Composer/Data/Scripts/' + script.id + '.json)', "Entry Point [" + node.entryPointId + "] not found in associated script: [" + node.scriptId + "]");
            }
        }
    };
